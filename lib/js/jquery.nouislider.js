							...
							if (!slider.hasClass('disabled')) {
								...
								$(document).on(moveEvent, function (f) {
									...
									if (movement[orientation] && proposal != previousProposal) {
										...
										call(settings.slide, slider.data('_n', true), {"val" : percentage.is(settings.range, proposal).toFixed(res)});
					...
					if (EVENT == 1) {
						slider.on('click', function (f) {
							if (!slider.hasClass('disabled')) {
								...
								call(settings.slide, slider, {"val" : percentage.is(settings.range, correct(proposal, slider, handle)).toFixed(res)});
								slider.change();
							}
						});
					}
